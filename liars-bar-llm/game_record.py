...
class RoundRecord:
    ...
    def get_play_decision_info(self, self_player: str, interacting_player: str) -> str:
        ...
        """
        self_gun = next((ps.current_gun_position for ps in self.player_initial_states if ps.player_name == self_player),
                        None)
        other_gun = next(
            (ps.current_gun_position for ps in self.player_initial_states if ps.player_name == interacting_player),
            None)
...
class GameRecord:
    ...
    def start_round(self, round_id: int, target_card: str, round_players: List[str], starting_player: str,
                    player_initial_states: List[PlayerInitialState],
    ...
    def record_play(self, player_name: str, played_cards: List[str], remaining_cards: List[str], play_reason: str,
                    behavior: str, next_player: str, play_thinking: str = None) -> None:
        """记录玩家的出牌行为"""
    ...
    def record_challenge(self, was_challenged: bool, reason: str = None, result: bool = None,
                         challenge_thinking: str = None) -> None:
        """记录质疑信息"""
        current_round = self.get_current_round()
        if current_round:
